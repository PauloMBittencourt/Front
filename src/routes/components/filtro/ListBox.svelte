<script lang="ts">
	...
	export let itemSelected: Filter;
	import {
		Listbox,
		ListboxButton,
		ListboxOptions,
...
<div class="col-span-2">
	<Listbox value={itemSelected} on:change={(e) => (itemSelected = e.detail)} class="absolute group">
		<ListboxButton class="text-lg text-vermelho-desty font-semibold pl-2 pr-2 flex flex-row "
			>{itemSelected.name}
			<svg
		...
		>
			{#each list as filter (filter.id)}
				<ListboxOption
					...
					class="p-2 hover:bg-red-50 hover:rounded-lg"
					let:selected
				>
					{#if selected}
